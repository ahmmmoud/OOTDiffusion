...
from run import model_runner_lower

is_garment_received = False
processed = False

def start_server(host='localhost', port=29999):
    ...
    while True:
        ...
        try:
            ...
            if(is_garment_received):
                model_runner_lower.run(model_path, garment_path)
                processed = True
                is_garment_received = False
            else:
...
def send_image():
    ...
    client_socket.connect(('localhost', 28888))  # Connect to server
